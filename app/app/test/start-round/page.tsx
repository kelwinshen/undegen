...
export default function StartRoundTest() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (!configInfo || !configInfo.data.slice(0, 8).equals(LOTTERY_CONFIG_DISCRIMINATOR)) {
        throw new Error("LotteryConfig not found — initialize the lottery first.");
      }
      ...
      addLog("info", `Jackpot token account: ${jackpotTokenAccount.toBase58()}`);
      ...
      setResult({ type: "success", message: `Round ${roundId} started! Tx: ${sig}` });
    } catch (err: any) {
      ...
      <main className="relative z-10 mx-auto flex min-h-screen max-w-2xl flex-col gap-8 border-x border-border-low px-6 py-12">
        ...
        <div className="p-6 bg-bg2 rounded-xl border border-border-low space-y-6">
          ...
          <p className="text-sm text-gray-400">
            Starts the next lottery round (current_round_id + 1), creating its Round account and jackpot token account. Requires LotteryConfig to already exist.
          </p>
