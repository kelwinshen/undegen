...
const SETTLE_DEFAULT_DISCRIMINATOR = new Uint8Array([
...
export default function SettleDefault() {
  ...
  const fetchBatch = async () => {
    ...
    try {
      ...
      let decoded;
      try {
        decoded = BatchLayout.decode(accountInfo.data.slice(8));
      } catch (decodeErr: any) {
        throw new Error(
  ...
  const handleSettleDefault = async () => {
    ...
    try {
      ...
      const operator = getOperatorKeypair(); // only for fee payment, not a required signer
    ...
    <div className="relative min-h-screen overflow-x-clip bg-bg1 text-foreground">
      <main className="relative z-10 mx-auto flex min-h-screen max-w-3xl flex-col gap-8 border-x border-border-low px-6 py-12">
        ...
        <div className="p-6 bg-bg2 rounded-xl border border-border-low space-y-6">
          <h2 className="text-xl font-bold">Settle Default (Test)</h2>
          <p className="text-sm text-gray-400">
            Load a batch and trigger settle_default. The contract will reject if
            the batch is not in <strong>AwaitingCollateral</strong> state or the
            collateral deadline has not passed.
          </p>
          ...
          {batchData && (
            <div className="space-y-4">
              <div className="text-sm text-gray-400 p-3 bg-black/20 rounded-lg border border-border-low">
                ...
                <span className="block">
                  <strong>Collateral Deposited:</strong>{" "}
                  {batchData.collateral_deposited.toString()}
                </span>
                <span className="block">
              ...
              <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg text-sm text-blue-300">
                The button is always enabled once a batch is loaded. The program
                itself will enforce state requirements (e.g. must be in
                AwaitingCollateral, deadline passed). If the transaction fails,
                the error will appear below.
              </div>

              <button
                onClick={handleSettleDefault}
                disabled={!batchData || loading}
                className="w-full mt-4 px-6 py-3 bg-orange-500 text-black font-semibold rounded-lg hover:bg-orange-400 transition disabled:opacity-50"
              >
                {loading ? "Settling..." : "Settle Default"}
              </button>
            </div>
          )}
