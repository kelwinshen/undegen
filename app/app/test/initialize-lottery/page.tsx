...
export default function InitializeLotteryTest() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      if (
        !info ||
        !info.data.slice(0, 8).equals(LOTTERY_CONFIG_DISCRIMINATOR)
      ) {
        addLog("warning", "LotteryConfig not found — not yet initialized.");
        setResult({
          type: "error",
          message: "LotteryConfig does not exist yet.",
        });
        return;
      }
  ...
  const handleInit = async () => {
    ...
    try {
      ...
      setResult({
        type: "success",
        message: `Lottery initialized! Tx: ${sig}`,
      });
    } catch (err: any) {
      addLog("error", err.message);
      setResult({ type: "error", message: err.message });
      ...
      <main className="relative z-10 mx-auto flex min-h-screen max-w-2xl flex-col gap-8 border-x border-border-low px-6 py-12">
        ...
        <div className="p-6 bg-bg2 rounded-xl border border-border-low space-y-6">
          ...
          <p className="text-sm text-gray-400">
            Creates the LotteryConfig account for the devnet USDC mint. Must be
            done once before start_round.
          </p>
