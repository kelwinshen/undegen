...
export default function InitializeBatchTest() {
  ...
  const handleInitialize = async () => {
    ...
    addLog(
      "info",
      `Weekly rate: ${(weeklyRate * 100).toFixed(4)}% → ${weeklyBps} bps`
    );
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          const vaultConfig = VaultConfigLayout.decode(
            vaultConfigInfo.data.slice(8)
          );
          const totalUnderlying = vaultConfig.total_underlying;
          const requiredWeeklyTick = Number(totalUnderlying) * weeklyRate;
          addLog(
            "success",
            `Vault TVL: ${Number(totalUnderlying).toLocaleString()} raw units`
          );
          addLog(
            "info",
            `Required weekly tick_yield: ${requiredWeeklyTick.toFixed(0)} raw units`
          );
        } else {
      ...
      const data = Buffer.concat([
        INIT_BATCH_DISCRIMINATOR,
        writeU16LE(weeklyBps),
      ]);
      const keys = [
        { pubkey: operator.publicKey, isSigner: true, isWritable: true },
      ...
      <main className="relative z-10 mx-auto flex min-h-screen max-w-2xl flex-col gap-8 border-x border-border-low px-6 py-12">
        ...
        <div className="p-6 bg-bg2 rounded-xl border border-border-low space-y-6">
          ...
          <p className="text-sm text-gray-400">
            Creates a new batch with a fixed annual APY of{" "}
            <strong>{ANNUAL_APY_PERCENT}%</strong>. Protocol config is
