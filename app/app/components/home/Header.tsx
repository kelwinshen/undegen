  ...
  return (
    <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border-low pb-6">
      <div>
        ...
        <p className="text-sm text-muted mt-1">The Daily Prediction Syndicate. Zero principal risk.</p>
      </div>
      ...
      <div className="flex flex-col items-end gap-2 w-full md:w-auto">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => router.push("/")}
            className={`text-xs underline ${!isLobby ? "text-emerald-300 font-medium" : "text-gray-400 hover:text-gray-300"}`}
          >
            Active Batch (1)
          </button>
          <button
