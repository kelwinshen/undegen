...
interface SyndicateSidebarProps {
  ...
  joinedBatches?: Array<{
    ...
    weeklyYieldPool: number;
    totalDeposited: number;
    voteStatus?: "voting" | "voted" | "ongoing" | "waiting";
  }>;
...
export default function SyndicateSidebar({
  ...
  const voteStatusLabel = (status?: "voting" | "voted" | "ongoing" | "waiting") => {
    switch (status) {
      case "voting":
        return "Voting";
      case "voted":
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right flex items-center gap-1.5">
                        ...
                        >
                          {isCurrent ? "Selected" : voteStatusLabel(b.voteStatus)}
                        </span>
                      </div>
                    </button>
                  );
        ...
        <div className="p-3 border border-border-low rounded-lg space-y-2">
          ...
          <div className="w-full bg-neutral-200 dark:bg-gray-700 rounded-full h-2">
            <div
              ...
              style={{
                width: `${allocatedBudget > 0 ? ((allocatedBudget - remainingBudget) / allocatedBudget) * 100 : 0}%`,
              }}
            />
          </div>
          <div className="flex justify-between text-xs text-muted">
            <span>Allocated: {allocatedBudget.toFixed(AMOUNT_DECIMALS)} USDC</span>
        ...
        {isConnected && (
          <div className="p-3 border border-border-low rounded-lg space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono">{userWeeklyYield.toFixed(AMOUNT_DECIMALS)} USDC</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted">Expected Reward</span>
              <span className="font-mono">
                {yourPotentialReward.toFixed(AMOUNT_DECIMALS)} USDC
              </span>
            </div>
            <div className="flex justify-between text-sm">
