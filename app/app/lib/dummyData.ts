...
function generateSimulatedVotes(fixtures: Fixture[], batchStart: number) {
  ...
  fixtures.forEach((fixture) => {
    ...
    votes[skipId] = Math.floor(seededRandom(batchStart + fixture.fixtureId * 17 + 13) * 400) + 20;
  });
  return votes;
}
...
function computeMatchDecisions(
  ...
  for (const fixture of sorted) {
    ...
    fixture.options.forEach((opt) => {
      ...
      if (v > maxVotes) { maxVotes = v; winnerId = opt.id; }
    });
    ...
    const won = isBet ? seededRandom(batchStart + fixture.fixtureId * 7 + betsUsed) > 0.5 : false;
...
export function useDummyData(fixtures: Fixture[], batchStart: number | null) {
  ...
  const matchDecisions = computeMatchDecisions(fixtures, simulatedVotes, batchStart);
