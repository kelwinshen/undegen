    ...
    >
      {(title || description) && (
        <div className="max-w-7xl mx-auto py-12 px-4 md:px-8 lg:px-10">
          {title && (
            <h2 className="text-xl md:text-4xl mb-4 text-foreground max-w-4xl font-bold">
              {title}
            </h2>
          )}
          {description && (
            <div className="text-muted text-sm md:text-base max-w-2xl">
              {description}
            </div>
          )}
        </div>
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-linear-to-t from-neutral-900 to-transparent dark:from-white dark:to-transparent from-0% rounded-full"
          />
        </div>
      </div>
    </div>
